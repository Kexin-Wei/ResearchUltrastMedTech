...
def draw_ellipse(ax: plt.Axes,
                 width: float, height: float, angle: float = 0,
                 xc: float = 0, yc: float = 0,
                 **kwargs,
                 ) -> None:
    e = pat.Ellipse((xc, yc), width, height, angle=np.rad2deg(angle), fill=False, **kwargs)
    ax.add_patch(e)


def intersection_of_line_ellipse(k: float,
                                 c: float,
                                 a: float,
                                 b: float) -> list:
...
def draw_line(ax: plt.Axes, xStart: float, xEnd: float, c: float, k: float = 0, *args, **kwargs) -> None:
