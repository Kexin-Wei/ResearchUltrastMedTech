...
def read_xlsx(hifuType=1001, recordFileName: str = "scan_record.json", date=None, scanPlan=None, dataType=None) -> \
    ...
    if hifuType == "1001":
        ...
        xSpacing, ySpacing = np.abs([xPoints[0] - xPoints[1], yPoints[0] - yPoints[1]]).round(decimals=1)
...
def findDbArea(scanArray: np.ndarray, dbValue: float = -6) -> tuple[np.ndarray, np.ndarray, float]:
    """
    :param scanArray
    ...
    contours, hierarchy = cv2.findContours(biggestComponent, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_NONE)
...
def singleCircleMask2D(scanArray: np.ndarray, modelRadius: INT_OR_FLOAT, centerIj: LIST_OR_NUMPY,
                       spacings: LIST_OR_NUMPY) -> np.ndarray:
    """
    :param scanArray
    :param modelRadius
    :param centerIj
    ...
    if spacings[0] == spacings[1]:
        ...
        cv2.ellipse(mask, centerIj, axes, angle=0, startAngle=0, endAngle=360, color=1, thickness=cv2.FILLED)
    return mask
...
def egoThermalIntegral(scanArray: np.ndarray,
                       modelRadius: INT_OR_FLOAT,
                       centerIj: LIST_OR_NUMPY,
                       spacings: LIST_OR_NUMPY,
                       showFlag: bool = False) -> float:
    """
...
def packNeighbourLocation(modelRadiusImage: float,
                          centerIj: LIST_OR_NUMPY,
                          nNeighbour: int,
                          packingType: NEIGHBOUR_PACKING_TYPE = "hexagonal") -> np.ndarray:
    """
    ...
    if packingType == "hexagonal":
        ...
        neighbourLocs = np.array([[twoRadius, 0], [-twoRadius, 0], [modelRadiusImage, sqrtThreeRadius],
                                  [-modelRadiusImage, sqrtThreeRadius], [-modelRadiusImage, -sqrtThreeRadius],
                                  [modelRadiusImage, -sqrtThreeRadius]])  # type: ignore
    else:
        print("Packing type not supported yet.")
    return neighbourLocs + np.array(centerIj)


def singleNeighbourThermalIntegral(scanArray: np.ndarray, modelRadius: INT_OR_FLOAT, neighbourCenter: LIST_OR_NUMPY,
                                   spacings: LIST_OR_NUMPY) -> np.ndarray:
...
def neighbourThermalIntegral(scanArray: np.ndarray,
                             modelRadius: INT_OR_FLOAT,
                             centerIj: LIST_OR_NUMPY,
                             spacings: LIST_OR_NUMPY,
                             nNeighbour: int,
                             packingType: NEIGHBOUR_PACKING_TYPE = "hexagonal",
                             showFlag: bool = False) -> tuple[float, float]:
    modelRadiusImage = modelRadius / spacings[0]
    neighbourLocs = packNeighbourLocation(modelRadiusImage, centerIj, nNeighbour, packingType=packingType)
    neighbourThermal = 0
    if packingType == "hexagonal":
        if showFlag:
            ...
            for i in np.arange(6):
                ...
                iScan = singleNeighbourThermalIntegral(scanArray, modelRadius, iCenter, spacings)
                axes[i].imshow(iScan, cmap='gray')
                axes[i].plot([centerIj[0]], [centerIj[1]], marker='s', markerfacecolor='r', markeredgecolor='r')
        ...
        print("Neighbour thermal can't be calculated due to unsupported neighbour packing type.")
        return 0, 0
